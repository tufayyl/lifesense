// server.js — static site + /api/chat with health-only scope and hidden system prompt
...
app.use(express.static(path.join(__dirname))); // serves index.html, styles.css, script.js, chatbot.js
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (!inScope(clientMsgs)) return res.json({ reply: REFUSAL_TEXT });
...
// ---- start ----
app.listen(PORT, () => {
  console.log(`UI+API at http://localhost:${PORT}`);
});
